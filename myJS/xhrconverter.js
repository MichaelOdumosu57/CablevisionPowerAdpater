...
function xhrConvertFile(   dev_obj   ){
    ...
    return "<html>"+dev_obj.ccta.split(/"/).join("'").split(/'http:\/\/windsorempire.com\/[^']+\/|'http:\\\/\\\/windsorempire.com\\\/[^']+\\\//).join("'/CablevisionPowerAdpater/dependencies/index/")+"</html>"
    
    
    // (/'http:\/\/windsorempire.com\/[^']+\/|'http:\\\/\\\/windsorempire.com\\\/[^']+\\\/|http:\/\/windsorempire.com\/[^)]+ /)
